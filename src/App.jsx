...
// =====================================
// شريط نصائح المستودع المتحرك[cite: 3]
// =====================================
const InfoTicker = ({ tips }) => {
  ...
  const tipElements = tips.map((tip, index) => (
    <span key={index} className="flex items-center gap-3 px-8 whitespace-nowrap" dir="rtl">
        {(tip.includes('تحذير') || tip.includes('تنبيه') || tip.includes('⚠️') || tip.includes('🛑')) ? <span className="text-rose-400 text-sm">●</span> : <span className="text-cyan-400 text-sm">●</span>}
        {tip}
    </span>
  ));
  ...
  return (
    <div className="w-full bg-[#05080f] border-t border-[#162235] h-12 flex items-center overflow-hidden relative z-50 shrink-0" dir="ltr">
      ...
      <div className="ticker-track text-[15px] font-bold text-slate-300 tracking-wide" style={{ animationDuration: `180s` }}>
...
export default function App() {
  ...
  // فصل الأدوية لفوري ومستقبلي بناءً على سبب السحب (مجرد لوجيك بسيط)
  const immediateDrugs = drugs.filter(d => d['سبب_السحب'] && d['سبب_السحب'].includes("وزارة الصحة"));
  const futureDrugs = drugs.filter(d => d['سبب_السحب'] && !d['سبب_السحب'].includes("وزارة الصحة"));
...
// كرت الدواء (بديلاً لكرت السيارة)[cite: 3]
      ...
      <div className={`backdrop-blur-md border ${glow} rounded-2xl p-5 flex flex-col justify-between transition-all duration-300 hover:scale-[1.02] group w-full shadow-2xl relative overflow-hidden`}>
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-center mb-5">
            <span className="font-mono text-[10px] text-slate-400 bg-slate-900/80 px-2.5 py-1 rounded border border-slate-800 flex items-center gap-1"><IconCpu /> DB-ID #{drug['ID']}</span>
          ...
          <div className="mb-5 border-b border-slate-800/50 pb-4">
            <h3 className="font-black text-white text-xl tracking-wide mb-1.5" dir="ltr">{drug['اسم_الدواء']}</h3>
            <div className="flex items-center gap-2">
          ...
          <div className="flex items-center justify-between bg-[#0a101d]/80 backdrop-blur-sm border border-[#162235] rounded-xl px-4 py-3 mb-5">
            <div className="flex-[2]">
              ...
              <span className="font-mono text-cyan-400 font-black tracking-widest whitespace-nowrap text-sm">{drug['رقم_الباتش (Batch)']}</span>
            </div>
          ...
          <div className="bg-[#090d16]/80 p-3.5 rounded-xl border border-[#142033] mb-5">
            ...
            <div className="text-slate-200 leading-relaxed font-mono font-bold text-sm bg-slate-900 px-2 py-1 rounded inline-block border border-slate-700">{drug['تاريخ_الانتهاء'].substring(0,10)}</div>
          </div>
        </div>
      </div>
    );
...
// شاشة الإرسال الذكية (تصميم سايبربانك)
const SubmitOrderForm = ({ drugs }) => {
  ...
  const toggleDrug = (dStr) => {
      setSelectedDrugs(prev => prev.includes(dStr) ? prev.filter(x => x !== dStr) : [...prev, dStr]);
  };
      ...
      <div className="w-full bg-[#070b12]/80 backdrop-blur-md border border-[#121e30] rounded-2xl p-8 shadow-2xl relative overflow-hidden">
        ...
        {success ? (
            <div className="text-center py-20 relative z-10">
                <div className="w-24 h-24 bg-emerald-500/20 rounded-full flex items-center justify-center mx-auto mb-6 border border-emerald-500/50 shadow-[0_0_50px_rgba(16,185,129,0.4)] animate-pulse">
                    <IconCheck />
                </div>
            ...
            <form onSubmit={handleSubmit} className="relative z-10 space-y-8">
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-60 overflow-y-auto pr-2">
                        {drugs.map((d, i) => {
                            const dStr = `${d['اسم_الدواء']} (Patch: ${d['رقم_الباتش (Batch)']})`;
                            ...
                            return (
                                <div key={i} onClick={() => toggleDrug(dStr)} className={`cursor-pointer p-4 rounded-xl border transition-all ${isSelected ? 'bg-emerald-500/10 border-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.2)]' : 'bg-[#090d16] border-[#162235] hover:border-slate-600'}`}>
                                    <div className="flex items-center gap-3">
                                        <div className={`w-5 h-5 rounded flex items-center justify-center border ${isSelected ? 'bg-emerald-500 border-emerald-400 text-black' : 'border-slate-600'}`}>
                                            {isSelected && <IconCheck />}
                                        </div>
                                        <div>
                                            <div className={`font-black ${isSelected ? 'text-emerald-400' : 'text-slate-300'}`}>{d['اسم_الدواء']}</div>
                                            <div className="text-xs font-mono text-slate-500 mt-1">Batch: {d['رقم_الباتش (Batch)']}</div>
                                        </div>
                                    </div>
                                </div>
                            );
